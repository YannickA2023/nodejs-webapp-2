pipeline {
    ...
    environment {
    ACR_NAME = "yannickacr"
    registyUrl = "yannickacr.azurecr.io"
    IMAGE_NAME = "nodejswebapp:16"
    IMAGE_TAG = "v1.0.0"
    registryCredential  = "yannick-acr"
    }
    stages { 
        stage('SCM Checkout') {
            steps{
           git branch: 'main', url: 'https://github.com/YannickA2023/nodejs-webapp-2.git'
            }
        }
        // run sonarqube test
        stage('Run Sonarqube') {
